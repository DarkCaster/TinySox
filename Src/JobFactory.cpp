...
std::vector<std::shared_ptr<IJob>> JobFactory::CreateJobsFromResult(const IJobResult &source)
{
    ...
    if(source.resultType==JR_TERMINAL)
    {
        ...
        return std::vector<std::shared_ptr<IJob>>(0);
    }
    ...
    if(source.resultType==JR_NEW_CLIENT)
    {
        ...
        return std::vector<std::shared_ptr<IJob>>(1,std::make_shared<Job_ClientHandshake>(commService,commManager,config,State().AddHandler(result.handlerID)));
    }
    ...
    if(source.resultType==JR_CLIENT_MODE_CONNECT)
    {
        ...
        std::vector<std::shared_ptr<IJob>> jobs({std::make_shared<Job_TCPTunnel>(commManager,result.state,config,true),
         std::make_shared<Job_TCPTunnel>(commManager,result.state,config,false)});
        return jobs;
    }
